...
from main import ChatPDFProcessor
import tempfile
import os
import time
...
st.set_page_config(page_title="RAG with Mistral and Tavily")
...
def process_input():
    ...
    if st.session_state["user_input"] and len(st.session_state["user_input"].strip()) > 0:
        ...
        with st.session_state["thinking_spinner"], st.spinner("Thinking..."):
            try:
                agent_text = st.session_state["assistant"].query_document(
                    user_text,
...
def page():
    ...
    if len(st.session_state) == 0:
        ...
        if not tavily_api_key:
            ...
            return
        st.session_state["assistant"] = ChatPDFProcessor(tavily_api_key=tavily_api_key)

    st.header("RAG with Mistral and Tavily")
